...
import { DeleteEntryWithAuditReason, Entry, EntryType, PostEntryChildrenEntryType, PostEntryChildrenRequest, Shortcut } from '../../src/index.js';
...
describe("Create Copy Entry Tests", () => {
    ...
    test("Create Copy Entry Create Folder", async () => {
        ...
        let response: Entry = await _RepositoryApiClient.entriesClient.createOrCopyEntry({repoId,entryId: parentEntryId,request,autoRename:true});
    ...
    test("Create Copy Entry Create Shortcut",async()=>{
        ...
        response = await _RepositoryApiClient.entriesClient.createOrCopyEntry({repoId,entryId: parentEntryId,request,autoRename:true});
    ...
    test("Create Copy Entry Copy Shortcut",async()=>{
        ...
        request.entryType = PostEntryChildrenEntryType.Folder;
        request.name = newEntryName;
        let response = await _RepositoryApiClient.entriesClient.createOrCopyEntry({repoId,entryId: parentEntryId,request,autoRename:true});
        let targetentry = response.toJSON();
        expect(targetentry).not.toBeNull();
        createdEntries.push(targetentry);
        expect(parentEntryId).toBe(targetentry.parentId);
        ...
        let newEntry:Entry = await _RepositoryApiClient.entriesClient.createOrCopyEntry({repoId,entryId: parentEntryId,request,autoRename:true});
